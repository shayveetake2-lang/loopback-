...
function setUnreadBadge(conversations) {
  ...
  document.querySelectorAll('[data-unread-count]').forEach((badge) => { badge.textContent = unreadCount > 99 ? '99+' : String(unreadCount); });
}
...
async function settingsMenu() {
  ...
  document.querySelector('#settings-form').addEventListener('submit', async (event) => {
    ...
    preferences.theme = document.querySelector('#settings-theme-toggle').getAttribute('aria-pressed') === 'true' ? 'light' : 'dark';
...
function helpPage() {
  ...
  helpSurface.querySelector('.grid').insertAdjacentHTML('beforebegin', '<section class="glass mb-4 rounded-2xl p-5"><h2 class="font-display text-lg font-bold">Start here</h2><div class="mt-3 grid gap-3 text-sm leading-6 text-slate-400 sm:grid-cols-2"><p><strong class="text-slate-200">Add one person:</strong> Select Add contact, fill in their name and any details you know, then choose Add contact.</p><p><strong class="text-slate-200">Add a list:</strong> Select Import CSV and choose a plain <code class="text-fuchsia-200">.csv</code> text file exported from a spreadsheet such as Excel, Numbers, or Google Sheets.</p><p><strong class="text-slate-200">CSV layout:</strong> Put column names in the first row and one contact on each following row. Only <code class="text-fuchsia-200">name</code> is required.</p><p><strong class="text-slate-200">Useful columns:</strong> <code class="text-fuchsia-200">last_interaction_date</code>, <code class="text-fuchsia-200">last_topic</code>, <code class="text-fuchsia-200">relationship_tier</code>, <code class="text-fuchsia-200">role</code>, and <code class="text-fuchsia-200">company</code>.</p></div></section>');
  const helpDescriptions = {
    'Create an account': 'Choose a username, email, and password. Your username works like a nickname for signing in, while your email can also be used to sign in or reset your password.',
    'Add contacts': 'Use Add contact for one person, or Import CSV for a list. A CSV is a simple text file with commas between columns. Export it from Excel, Numbers, or Google Sheets as CSV, then make sure the first row says what each column is called.',
    'Read your dashboard': 'All Contacts shows every person you have saved. Drift Alerts shows only people who are overdue for a catch-up. The synced time tells you when LoopBack last received your contact data.',
    'Message people': 'Open Messages in the left menu to see all conversations. A number beside Messages means you have unread messages. Choose a conversation to read it, or use Write a new message to find another registered LoopBack user and start a private chat.',
    'Capture interactions': 'Capture message stores an important note after you speak with someone in another app or in person. It updates the last-contact date and topic, which keeps your reminders useful.',
    'Manage preferences': 'Open Settings to change your display name, use the Light/Dark switch, choose compact cards, or ask for a password reset email. Press Save changes to keep your choices on this device and in your account.',
  };
  helpSurface.querySelectorAll('article').forEach((article) => {
    const heading = article.querySelector('h2');
    const description = helpDescriptions[heading?.textContent.trim()];
    const paragraph = article.querySelector('p');
    if (description && paragraph) paragraph.textContent = description;
  });
  const explanations = {
...
function bootstrap() {
  ...
  const header = document.querySelector('header');
  refreshUnreadCount();
  if (header && !document.querySelector('#welcome-section')) header.insertAdjacentHTML('afterend', '<section id="welcome-section" class="glass mb-6 rounded-2xl border border-fuchsia-400/20 bg-fuchsia-500/5 p-5 sm:p-6"><div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><div class="text-[10px] font-bold uppercase tracking-[.2em] text-fuchsia-300">A better way to stay close</div><h2 class="mt-2 font-display text-2xl font-bold">Welcome to your relationship orbit.</h2><p class="mt-2 max-w-2xl text-sm leading-6 text-slate-400">LoopBack helps you remember the people who matter, notice when a connection needs care, and start your next conversation with confidence.</p></div><div class="flex shrink-0 flex-wrap gap-2"><button id="welcome-start" class="rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-500 px-3 py-2 text-xs font-bold text-white">Start here</button><button id="welcome-help" class="welcome-help rounded-xl border border-fuchsia-400/30 px-3 py-2 text-xs font-bold text-fuchsia-200 hover:bg-fuchsia-400/10">Explore the guide</button></div></div><div class="mt-5 grid gap-3 sm:grid-cols-3"><div class="rounded-xl bg-white/[.045] p-3"><div class="text-sm font-bold text-slate-200">Remember</div><p class="mt-1 text-xs leading-5 text-slate-500">Keep useful context in one calm place.</p></div><div class="rounded-xl bg-white/[.045] p-3"><div class="text-sm font-bold text-slate-200">Notice</div><p class="mt-1 text-xs leading-5 text-slate-500">See who may appreciate a thoughtful hello.</p></div><div class="rounded-xl bg-white/[.045] p-3"><div class="text-sm font-bold text-slate-200">Reconnect</div><p class="mt-1 text-xs leading-5 text-slate-500">Turn a reminder into a personal message.</p></div></div></section>');
  document.querySelector('#welcome-start')?.addEventListener('click', addContact);
  document.querySelector('#welcome-help')?.addEventListener('click', () => helpPage());
  const headerActions = header?.querySelector(':scope > div:nth-child(2)');
  if (headerActions && !document.querySelector('#add-contact')) headerActions.insertAdjacentHTML('beforeend', '<button id="add-contact" class="glass shrink-0 rounded-xl px-3 py-2 text-xs font-bold text-slate-300 hover:border-fuchsia-400/50">Add contact</button>');
